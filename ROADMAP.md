...
**Status:** Phases 0–6 complete — milestones `m01`–`m06` on disk, 238 unit tests, all phases re-run 2026-08-19 after the report/code alignment work, and the evaluation-side defects found in the two 2026-08-19 pre-Phase-7 audits closed (see the changelog: (ii) fixed the per-regime bucket **label**, (iii) fixed the transition **selector** and retracted the RQ3 transition headline). **Phase 7 (combined regime × uncertainty model + master results table) is next.**
