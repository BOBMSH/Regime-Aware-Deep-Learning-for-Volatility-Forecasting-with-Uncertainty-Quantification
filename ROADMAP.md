...
- **RQ3's retractions are independently confirmed.** The comparator's per-regime coverage on the t−1 label is **non-monotone** (MC-Dropout calm 0.848 / transitional 0.880 / crisis 0.887), like the headline's 0.844 / 0.894 / 0.842 — whereas its stale ex-post table (0.866 / 0.868 / 0.843) still showed the monotone-degradation pattern retracted in (ii). On the ex-ante transition selector there is again no separation (MC 0.848 flagged vs 0.866 stable; quantile 0.780 vs 0.825), confirming (iii) on the second estimator.
...
2. RQ3 is still descriptive: coverage is reported as point estimates with no hypothesis test. Kupiec (1995) unconditional coverage plus Christoffersen (1998) / Engle–Manganelli DQ — scheduled in Phase 7 — on the **full sample with t−1 regressors** (see 2026-08-19 (iii); *not* on a transition subsample).
