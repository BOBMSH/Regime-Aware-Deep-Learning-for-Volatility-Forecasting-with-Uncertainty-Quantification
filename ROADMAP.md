...
**Status:** Phases 0–6 complete — milestones `m01`–`m06` on disk, 230 unit tests, all phases re-run 2026-08-19 after the report/code alignment work, and the evaluation-side defects found in the 2026-08-19 pre-Phase-7 audit closed (see the changelog). **Phase 7 (combined regime × uncertainty model + master results table) is next.**
...
3. **RQ3 — intervals are over-confident, and they fail at regime *transitions*, not at regime *severity*.** MC-Dropout PICP 0.865 and quantile 0.821 at 90% nominal. The failure is concentrated on the 52 days (6.6%) when the state changes: **MC-Dropout 0.712 vs 0.876 on stable days; quantile 0.673 vs 0.832.** *This supersedes the earlier "coverage degrades monotonically to 0.813/0.773 in crisis" claim, which was an artefact of bucketing days by a regime label inferred from the same day's return — see the 2026-08-19 (ii) changelog entry.* Under the correctly-timed t−1 label the per-regime ordering is non-monotone (MC-Dropout calm 0.844 / transitional 0.894 / crisis 0.842) and calm-vs-crisis is inside sampling noise.
...
### Phase 6 — Uncertainty quantification (Weeks 8–9)  ·  ✅ **DONE** — MC-Dropout PICP 0.865 & quantile 0.821 at 90% (both mildly over-confident); **the failure is concentrated at regime *transitions*: MC-Dropout 0.712 vs 0.876 on stable days, quantile 0.673 vs 0.832, on the 52 days (6.6%) when the state changes.** Per-regime coverage under the correctly-timed t−1 label is *non-monotone* (MC calm 0.844 / transitional 0.894 / crisis 0.842); the earlier "degrades monotonically to 0.813/0.773 in crisis" reading was an artefact of bucketing by a same-day-inferred label and is retracted (2026-08-19 (ii)). Point accuracy unchanged vs the Phase-3 LSTM (DM p=0.19) (`m06_uq.md`)
...
- Calibration *per regime* — this is where UQ usually breaks and where the contribution lies — *done*, and the result is not the one originally expected. Bucketed by the regime known at **t−1** (the only timing that supports a conditional claim — see 2026-08-19 (ii)), coverage is **not** monotone in regime severity. The robust, convention-free version is the **transition vs stable** split, where both methods shed ~16 percentage points of coverage on state-change days. The MC-Dropout log-normal band is near-homoskedastic on the log scale so it cannot widen *ahead* of a transition; the quantile head can widen but keys off the same lagged inputs and is caught by the same one-day surprise — the substantive RQ3 contrast.
...
- **NEW — make RQ3 inferential, not descriptive.** The calibration claim is still point estimates of PICP with no hypothesis test, exactly the gap Giacomini–White closed for RQ2. Add an unconditional-coverage test (Kupiec 1995 LR), ideally with Christoffersen's (1998) conditional-coverage extension or the Engle–Manganelli dynamic quantile test *already cited in Ch2 §2.6*. **Apply it to the transition-vs-stable split, not the per-regime split** — after the 2026-08-19 (ii) fix that is where the effect actually lives (0.712 vs 0.876 at 90% nominal), and Christoffersen's independence component is the natural test for it, since clustered interval failures at state changes are precisely what it is designed to detect. Reporting "0.712 vs 0.876" with a p-value is the deliverable.
...
An independent audit of Chapters 1–2 against the implemented pipeline (`AUDIT_2026-08-19_pre_phase7.md`) verified that every headline number reproduces exactly from the persisted parquets and that the leakage discipline holds, and found one methodological defect plus a set of smaller ones. All are now closed.
...
**Still open:**
1. `refit_every_folds: 1` sensitivity for the best LSTM, to separate architecture from training-window staleness (Phase 8).
2. RQ3 is still descriptive: coverage is reported as point estimates with no hypothesis test. Kupiec (1995) unconditional coverage plus Christoffersen (1998) / Engle–Manganelli DQ — already scheduled in Phase 7 — should now be applied to the **transition vs stable** split, which is where the effect actually is.
